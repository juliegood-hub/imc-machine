...
const SECTION_TAYLOR_ZONE_MAP = {
  ...
  chat_assistant: ['people', 'purpose_program'],
  production_ops: ['people', 'place_stuff', 'purpose_program'],
  safety_risk: ['people', 'money', 'place_stuff', 'purpose_program'],
...
const SECTION_PRODUCTION_PHASE_MAP = {
  ...
  chat_assistant: 'pre_production',
  production_ops: 'production',
  safety_risk: 'production',
...
export const WORKFLOW_SECTIONS = [
  ...
  },
  {
    ...
    summary: 'Build and share professional plots, then run show execution with checklists and department plans.',
    steps: [
      { title: 'Build plots + layer views', actor: 'human', desc: 'Configure stage/audio/LX layouts plus power and safety-egress views with custom labels.' },
      { title: 'Share, publish, and version plots', actor: 'review', desc: 'Set Owner/Editor/Commenter/Viewer permissions, publish, return to draft for edits, and republish.' },
      { title: 'Department modules', actor: 'human', desc: 'Manage costumes, set/scenery, dressing rooms, and parking/permits/maps.' },
      { title: 'Production checklists', actor: 'human', desc: 'Track load-in, before-doors, show call, strike, and after-show closeout readiness.' },
    ],
  },
  {
    id: 'safety_risk',
    ...
    productionPhase: 'production',
    taylorZones: ['people', 'money', 'place_stuff', 'purpose_program'],
    summary: 'Run risk scoring, permits, insurance, incidents, and emergency action plans from one safety command lane.',
    steps: [
      { title: 'Set risk profile + score', actor: 'review', desc: 'Capture attendance, alcohol, weather, staffing, and fire risk factors for dynamic risk scoring.' },
      { title: 'Track permits + insurance', actor: 'human', desc: 'Manage permit status, policy coverage, expiration alerts, and linked compliance files.' },
      { title: 'Map screening, surveillance, and medical readiness', actor: 'human', desc: 'Track CCTV/access points, crowd controls, first aid plans, and city coordination contacts.' },
      { title: 'Generate emergency action plan', actor: 'automatic', desc: 'Auto-generate EAP docs with chain of command, evacuation flow, and response protocols.' },
    ],
  },
  {
  ...
  },
  {
    id: 'chat_assistant',
    number: 10,
    title: 'Buddy the CatBot (Helpful Chat Assistant)',
    icon: '🐈‍⬛',
    track: 'booking_setup',
    productionPhase: 'pre_production',
    taylorZones: ['people', 'purpose_program'],
    summary: 'Ask Buddy the CatBot for fast guidance, next steps, and one-click links to the right page.',
    steps: [
      { title: 'Ask in plain language', actor: 'human', desc: 'Tell Buddy what you are trying to do in normal words; no special command format needed.' },
      { title: 'Get role-aware guidance', actor: 'automatic', desc: 'Buddy answers with role-aware steps for promoter, stage manager, legal coordinator, and venue workflows.' },
      { title: 'Receive one-click action links', actor: 'automatic', desc: 'Buddy suggests direct links to event create, IMC composer, production ops, staffing, and reporting pages.' },
      { title: 'Turn loose notes into clean next steps', actor: 'review', desc: 'Paste notes, and Buddy can return a practical checklist before you execute.' },
    ],
  },
];
